...
public class ShootTextProController : MonoBehaviour
{
    ...
    public Transform ShootTextCanvas
    {
        get
        {
            ...
            if (shootTextCanvas == null)
            {
                Debug.LogError("shootTextCanvas丢失");
            }
    ...
    void Update()
    {
        ...
        for (int i = 0; i < handleShootTextGroup.Count; i++)
        {
            ...
            shootTextCreatPosition = shootTextComponent.cacheTranform.GetComponent<Collider2D>().bounds.center + (((Vector3.up * shootTextComponent.cacheTranform.GetComponent<Collider2D>().bounds.size.y) * 0.5f));
    ...
    {
        Vector3 playerCenter = payerTranform.GetComponent<Collider2D>().bounds.center;
        Vector3 halfYVector3 = (Vector3.up * payerTranform.GetComponent<Collider2D>().bounds.size.y) * 0.5f;
